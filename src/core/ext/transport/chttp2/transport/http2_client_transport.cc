...
Http2Status Http2ClientTransport::ProcessHttp2SettingsFrame(
  ...
  auto Http2ClientTransport::ProcessOneFrame(Http2Frame frame) {
    ...
    return AssertResultType<Http2Status>(MatchPromise(
        ...
        [self =
             RefAsSubclass<Http2ClientTransport>()](Http2SettingsFrame frame) {
        ...
        [self =
             RefAsSubclass<Http2ClientTransport>()](Http2SecurityFrame frame) {
  ...
  auto Http2ClientTransport::ReadAndProcessOneFrame() {
    ...
    return AssertResultType<absl::Status>(TrySeq(
        ...
        [self =
             RefAsSubclass<Http2ClientTransport>()](Http2FrameHeader header) {
        ...
        [self = RefAsSubclass<Http2ClientTransport>()](
          ...
          ValueOrHttp2Status<Http2Frame> frame = ParseFramePayload(
              self->current_frame_header_, std::move(payload));
          if (!frame.IsOk()) {
  ...
  auto Http2ClientTransport::OnWriteLoopEnded() {
    ...
    return [self = RefAsSubclass<Http2ClientTransport>()](absl::Status status) {
      GRPC_HTTP2_CLIENT_DLOG
          << "Http2ClientTransport OnWriteLoopEnded Promise Status=" << status;
      GRPC_UNUSED absl::Status error =
          self->HandleError(Http2Status::AbslConnectionError(
              status.code(), std::string(status.message())));
    };
  ...
  Http2ClientTransport::Http2ClientTransport(
    ...
    const int max_hpack_table_size =
        channel_args.GetInt(GRPC_ARG_HTTP2_HPACK_TABLE_SIZE_ENCODER)
            .value_or(-1);
  ...
  void Http2ClientTransport::CloseStream(
      uint32_t stream_id, absl::Status status, CloseStreamArgs args,
  ...
  RefCountedPtr<Http2ClientTransport::Stream>
  Http2ClientTransport::LookupStream(uint32_t stream_id) {
    MutexLock lock(&transport_mutex_);
        ...
        TrySeq(
            ...
            [self = RefAsSubclass<Http2ClientTransport>(),
             stream_id]() mutable {
              ...
              Http2DataFrame frame{stream_id, /*end_stream*/ true,
                                   SliceBuffer()};
              return self->EnqueueOutgoingFrame(std::move(frame));
            },
